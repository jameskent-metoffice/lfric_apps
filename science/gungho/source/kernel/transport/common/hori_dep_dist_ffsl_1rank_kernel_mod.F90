...
!!          This kernel is implemented for use on a single MPI rank, as if-
!!          checks are embedded deep in the code to identify when calculations
!!          have crossed particular panel boundaries.
module hori_dep_dist_ffsl_1rank_kernel_mod
...
type, public, extends(kernel_type) :: hori_dep_dist_ffsl_1rank_kernel_type
  private
...
contains
  procedure, nopass :: hori_dep_dist_ffsl_1rank_code
end type

!-------------------------------------------------------------------------------
! Contained functions/subroutines
!-------------------------------------------------------------------------------
public :: hori_dep_dist_ffsl_1rank_code

contains

!> @brief Kernel to compute the horizontal departure distances for FFSL, for
!!        a single MPI rank
...
subroutine hori_dep_dist_ffsl_1rank_code( nlayers,             &
                                          dep_dist,            &
                                          frac_dry_flux,       &
                                          dry_flux,            &
                                          dry_mass_for_x,      &
  ...
  integer(kind=i_def) :: stencil_idx, col_idx, rel_idx, pid_idx
    ...
    do df_idx = 1, face_selector_ew(map_w3_2d(1))
      ...
      do k = 0, nlayers - 1
        ...
        do j = 1, stencil_extent
          ...
          ! Get mass for the next cell, depending on the panel
...
end subroutine hori_dep_dist_ffsl_1rank_code

end module hori_dep_dist_ffsl_1rank_kernel_mod
